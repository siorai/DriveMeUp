...
def serviceAccountCredential():
    """
    Utilizes the Service Account Credential flow to instantiate an authorized
    credentials object for calling the Google API
    """
    settings = settingsLoader()
    scopes = settings['scopes']
    settings = settings['serviceAccountCredentials']
    keyfile = settings['serviceKeyFile']
    logging.debug('AUTH: SAC_FLOW: Finished loading Service Account Keyfile:' +
                  ' using %s' % keyfile)
    ...
    logging.debug('AUTH: SAC_FLOW: Initializing credential from ' +
    ...
    delegated_credentials = (credentials.create_delegated
                             (settings['delegatedEmail']))
    logging.debug('AUTH: SAC_FLOW: Initializing authorized, delegated ' +
                  'credentials object')
